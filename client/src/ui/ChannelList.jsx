...
export default function ChannelList(){
  ...
  return (
    <div className="w-[280px] border-r border-white/10 bg-[#101822] h-full flex flex-col">
        ...
        {channels.map(ch=>{
          ...
          return (
            <button key={ch.id} onClick={()=>switchCh(ch.id)}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl ${isActive?'panel':'panel hover:bg-white/10'} transition`}>
              <span>＃ {ch.name}</span>
...
function E2EEControls({ channelId }){
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <button onClick={save} className="tg-button text-xs">Установить ключ</button>
      <button onClick={show} className="tg-button text-xs">Показать ключ</button>
